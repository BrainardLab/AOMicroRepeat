            ...
            for hh = 1:length(theSplits)
                ...
                for mm = 1:length(theMethods)
                    ...
                    [plot_log_intensities,plot_psychometric,corrected_psychometric, ...
                        log_threshold,corrected_log_threshold,psiParamsValues,h,correctForGuessing_log_threshold] = FitPsychometricFunction(trial_log_intensities,trial_responses,log0ValueUse,thresholdCriterion, ...
                    ...
                    theTableCorrectForGuessingLogThreshold(tableRow,1) = correctForGuessing_log_threshold;
...
fprintf('\nWriting xlsx file ... ')
if (convertToDb)
    tableVariableNames = {'Subject','Diameter','Session','Method','Split','Threshold (dB)','Corrected Threshold (dB)','Correct For Guessing Threshold (dB)',CI Low','CI High', 'Alpha','Beta','Guess','Lapse', ...
    ...
    tableVariableNames = {'Subject','Diameter','Session','Method','Split','Log10 Threshold','Corrected Log10 Threshold','Correct For Guessing Log10 Threshold','CI Low','CI High', 'Alpha','Beta','Guess','Lapse', ...
        'Criterion','SlopeLimitLow','SlopeLimitHigh','guessLimit','lapseLimit','guessEstFromData','nTrialsGuessEstFromData','lapseEstFromData','nTrialsLapseEstFromData'};
    thresholdPlaces = 2;
end
dataTable = table(theSubject,theDiameter,theSession,theMethod,theSplit, ...
    round(theTableLogThreshold,thresholdPlaces),round(theTableCorrectedLogThreshold,thresholdPlaces),round(theTableCorrectForGuessingLogThreshold,thresholdPlaces), ...
    round(theTableCorrectedBootCILow,thresholdPlaces), round(theTableCorrectedBootCIHigh,thresholdPlaces), ...
