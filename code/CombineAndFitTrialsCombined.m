...
theSplits = {'All', 'FirstHalf', 'SecondHalf' 'Combined'};
theSplits = {'Combined'};
        ...
        for ss = 1:length(theSessions)
            for hh = 1:length(theSplits)
                ...
                for mm = 1:length(theMethods)
                    ...
                    % Say hello
                    if (~justCheckFiles & ~(strcmp(theMethods{mm},'MOCS') & strcmp(theSplits{hh},'Combined')))
                        fprintf('Analyzing condition %d\n',tableRow);
                    end

                    % Store info for what we are analyzing in this run
                    theMethodOrig{tableRow,1} = theMethods{mm};
                    theSplitsOrig{tableRow,1} = theSplits{hh};
                    if (strcmp(theSplits{hh},'Combined'))
                        theMethod{tableRow,1} = 'Combined';
                        theSplit{tableRow,1} = 'All';
                    else
                    ...
                    pathToData = fullfile(dataDir,theSubject{tableRow},['Session' num2str(theSession(tableRow))],['Size' num2str(theDiameter(tableRow))],theMethodOrig{tableRow});
                    ...
                    for i = 1:num_trial_videos
                        ...
                        if (strcmp(theMethodOrig{tableRow},'MOCS'))
                        ...
                        elseif (isfield(tempData,'trial_matrix'))
                            if (~strcmp(theMethodOrig{tableRow},'QUEST'))
                    ...
                    if (strcmp(theMethodOrig{tableRow},'MOCS') & size(MOCSFileTimes,1) ~= 4)
                        error('Wrong number of MOCS files somewhere in data tree');
                    end
                    if (strcmp(theMethodOrig{tableRow},'QUEST')  & size(QUESTFileTimes,1) ~= 4)
                    ...
                    % If just checking, don't do anything else.  Also if
                    % we're only accumulating MOCS data for the Combined
                    % analysis.
                    if (~justCheckFiles & ~(strcmp(theMethods{mm},'MOCS') & strcmp(theSplits{hh},'Combined')))
                        ...
                        if (~strcmp(theMethod{tableRow},'Combined'))
                            for bb = 1:nBootstraps
                                if (rem(bb,100) == 0)
                            ...
                            boot_corrected_threshold = corrected_log_threshold*ones(nBootstraps,1);
                        end
                        boot_quantiles = [corrected_log_threshold corrected_log_threshold corrected_log_threshold];
                        figure(h);
                        ...
                        % data.
                        clear all_trials
                         if (~strcmp(theMethod{tableRow},'Combined') | strcmp(theMethods{mm},'QUEST'))
